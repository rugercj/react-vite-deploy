...
export default function ExpensesMaster() {  
    ...
    const [formData, setFormData] = useState({ expensesname: "" });
    ...
    // --- Real-time listener
  useEffect(() => {
    const q = query(collection(db, "expensesmaster"), orderBy("createdAt", "asc"));
  ...
  const goToPreviousPage = () => {
    ...
    const handleSubmit = async () => {
      try {
        ...
        if (isEditMode && currentStaffId) {
          // UPDATE staff
          await updateDoc(doc(db, "expensesmaster", currentStaffId), {
          ...
          // ADD staff
          await addDoc(collection(db, "expensesmaster"), {
            expensesname: formData.expensesname,
            createdAt: serverTimestamp()
          });
    ...
    const handleConfirmDelete = async () => {
      ...
      try {
        await deleteDoc(doc(db, "expensesmaster", staffToDelete.id)); // make sure your Firestore collection is "staff"

         // Remove from local state immediately
        setItems(prevItems => prevItems.filter(i => i.id !== staffToDelete.id));
        console.log("Expense/s deleted successfully");
      } catch (error) {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.infoBlock}>
        <Text style={styles.username}>{item.expensesname}</Text>
      </View>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Text style={styles.screenTitle}>Expenses Management</Text>
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        <ScrollView>
                            ...
                            <TextInput placeholder="Expenses Name *" placeholderTextColor="#888" value={formData.expensesname} onChangeText={text => setFormData({...formData, expensesname: text})} style={[styles.stafffields, {marginTop:14}]}/>
