...
export default function Suppliers() {  
    ...
    const renderItem = ({ item }) => (
        <View style={styles.card}>
            ...
            <View style={styles.infoBlock}>
                <Text style={styles.username}>{item.suppliername}</Text>
                <View style={styles.inlineInfo}>
                    {item.email ? <Text style={styles.staffInfo}><Text style={{fontWeight:100}}>Email: </Text>{item.email}</Text> :null}
                    {item.mobile ? <Text style={[styles.staffInfo, { borderRightWidth: 0 }]}><Text style={{fontWeight:100}}>Mobile: </Text>{item.mobile}</Text> : null}
                    {item.remarks ? <Text style={[styles.staffInfo, { borderRightWidth: 0 }]}><Text style={{fontWeight:100}}>Remarks: </Text>{item.remarks}</Text> : null}
                </View>
            </View>
    ...
    const handleSubmit = async () => {
      try {
        ...
        if (isEditMode && currentStaffId) {
          // UPDATE staff
          await updateDoc(doc(db, "suppliers", currentStaffId), {
    ...
    const handleConfirmDelete = async () => {
      ...
      try {
        await deleteDoc(doc(db, "suppliers", staffToDelete.id)); // make sure your Firestore collection is "staff"
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Text style={styles.screenTitle}>Suppliers Management</Text>
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        <ScrollView>
                            ...
                            <TextInput placeholder="Supplier Name *" placeholderTextColor="#888" value={formData.supplierName} onChangeText={text => setFormData({...formData, supplierName: text})} style={[styles.stafffields, {marginTop:14}]}/>
                            <TextInput placeholder="Mobile *" placeholderTextColor="#888" value={formData.mobile} onChangeText={text => setFormData({...formData, mobile: text})} style={styles.stafffields}/>
                            <TextInput placeholder="Email *" placeholderTextColor="#888" value={formData.email} onChangeText={text => setFormData({...formData, email: text})} style={styles.stafffields}/>
                            <TextInput placeholder="Remarks *" placeholderTextColor="#888" value={formData.remarks} onChangeText={text => setFormData({...formData, remarks: text})} style={styles.stafffields}/>
